...
for datatype in tqdm(
    ...
    for state in tqdm(states):
        ...
        for space in ["explanation", "input", "prediction"]:
            ...
            auc_tr = roc_auc_score(y_cal_2, detector.model.predict(X_cal_2))
            ...
            if space == "prediction":
                X_ood_te_["pred"] = detector.model.predict(X_ood_te)
            else:
                ...
                for N in [20_000, 5_000, 1_000, 500, 100]:
                    try:
                        auc_ood = roc_auc_score(
                            X_ood_te_.head(N).y,
                            # detector.model.predict_proba(X_ood_te_.head(N).drop(columns=["y", "pred"]))[:, 1],)
