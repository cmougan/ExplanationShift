...
for state in tqdm(states):
    ...
    for space in ["explanation", "input", "prediction"]:
        ...
        auc_ood = roc_auc_score(
            X_ood_te_.head(N).y,
            detector.model.predict_proba(X_ood_te_.head(N).drop(columns=["y", "pred"]))[
                :, 1
            ],
        )
        res.append([space, state, auc_tr - auc_ood])

results = pd.DataFrame(res, columns=["space", "state", "auc_diff"])
# %%
# Convert results to table with State vs Space
results = results.pivot(index="state", columns="space", values="auc_diff")
