...
# %%
data = GetData(type="real", datasets="ACSTravelTime")
...
# OOD Data
X_ood, y_ood = data.get_state(state="HI", year="2018")
...
for space in ["explanation", "input", "prediction"]:
    ...
    auc_te = np.round(roc_auc_score(y_te, detector.model.predict_proba(X_te)[:, 1]), 3)
    auc_ood = np.round(
