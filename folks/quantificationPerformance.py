...
def create_meta_data(test, samples, boots):
    ...
    for i in tqdm(range(0, boots), leave=False, desc="Test Bootstrap", position=1):
        ...
        for feat in ca_features.columns:
            ...
            row_shap.append(sh)
        # Target shift
        ks_target_shift = np.mean(preds_ca) - np.mean(preds)
...
for state in tqdm(states):
    ...
    try:
        ...
        X_tr, X_te, y_tr, y_te = train_test_split(
            input_tr, model_error_tr, test_size=0.9, random_state=42
        )
        ...
        # Shap
        X_tr, X_te, y_tr, y_te = train_test_split(
            shap_tr, model_error_tr, test_size=0.9, random_state=42
        )
        clf.fit(X_tr, y_tr)
        shap_results = roc_auc_score(y_te, clf.predict_proba(X_te)[:, 1])
        # Output
        X_tr, X_te, y_tr, y_te = train_test_split(
            output_tr, model_error_tr, test_size=0.9, random_state=42
        )
        clf.fit(X_tr, y_tr)
