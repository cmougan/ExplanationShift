...
def test_doc_examples():
    ...
    XX_ood, _ = make_blobs(n_samples=1000, centers=1, n_features=5, random_state=0)

    detector = ExplanationShiftDetector(
        model=XGBRegressor(random_state=0), gmodel=LogisticRegression()
    )
    # On OOD
    detector.fit(X_source=XX_tr, y_source=yy_tr, X_ood=XX_ood)
    assert np.round(detector.get_auc_val(), decimals=2) == 0.77
    # On test
    detector.fit(X_source=XX_tr, y_source=yy_tr, X_ood=XX_te)
